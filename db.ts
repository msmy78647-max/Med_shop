...
import { InsertUser, users, orders, Order, InsertOrder } from "../drizzle/schema";
...
export async function updateOrderStatus(id: number, status: Order["status"]) {
  ...
  try {
    const result = await db.update(orders).set({ status, updatedAt: new Date() }).where(eq(orders.id, id));
    return result;
  } catch (error) {
